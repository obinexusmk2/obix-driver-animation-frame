...
interface GraphNode {
  id: number;
  type: AnimationNodeConfig['type'];
  config: AnimationNodeConfig;
  children: GraphNode[];
  timeline: TimelineHandle | null;
...
function playNode(node: GraphNode): void {
...
function pauseNode(node: GraphNode): void {
...
function cancelNode(node: GraphNode): void {
  node.state = 'finished';
  if (node.timeline) {
...
function finishNode(node: GraphNode): void {
  node.state = 'finished';
  node.progress = 1;
...
function updateNodeProgress(node: GraphNode): void {
  switch (node.type) {
    case 'timeline':
      if (node.timeline) {
...
export function createAnimationGraph(
  ...
  const graphs = new Map<number, GraphNode>();
